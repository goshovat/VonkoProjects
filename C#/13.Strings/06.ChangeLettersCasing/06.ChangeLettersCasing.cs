...
class ChangeLettersCasing
{
    static void Main()
    {
        ...
        {
            Console.WriteLine("Error with arrangement of tags. Details:\n{0}", applExc.Message);
        }
        catch (IndexOutOfRangeException indexOutRangeExc)
        {
            Console.WriteLine("Error! The index was out of range. Details:\n{0}", indexOutRangeExc.StackTrace);
        }
        catch (Exception e)
        {
        ...
        for (int i = 0; i < len; i++)
        {
            ...
            if (text[i] == '<')
            {
                ...
                if (i < len - 2)
                {
                    ...
                    {
                        //if we will open another tag, but the previous is not closed apparently there is a problem. our program does not support nested tags
                        if (currentTag != "")
                            throw new ApplicationException(String.Format("Unclosed tag! Check the text at index {0}", i));
                ...
                {
                    throw new ApplicationException(String.Format("Not a valid tag! Check the original text on index {0}", i));
                }
            }
            else if (text[i] == '>')
            {
                if (!inOpeningTag && !inClosingTag)
                    throw new ApplicationException(String.Format("Error! A tag is not correctly opened. Check the text at index {0}", i));

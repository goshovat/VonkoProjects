...
class InputIntegers
{
    ...
    static int ReadInteger(int start, int end)
    {
        ...
        {
            throw new ArgumentOutOfRangeException("Number is out of range! It should be between" + start + " and " + end);
        }
    ...
    static List<int> GetSequence(int start, int end)
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            if (start == 99)
            {
                throw new ApplicationException("We cannot put another integer between " + start + " and " + (int)(end + 1));
            }
            if (start == 100)
            {
                throw new ApplicationException("We cannot put another integer between " + start + " and " + (int)(end + 1));
